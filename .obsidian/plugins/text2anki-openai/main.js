...
__export(main_exports, {
  default: () => FlashcardGeneratorPlugin
});
...
var FlashcardGeneratorPlugin = class extends import_obsidian.Plugin {
  async onload() {
  ...
  async generateFlashcardsFromCurrentFile() {
    ...
    if (!text) {
      new import_obsidian.Notice(`Couldn't retrieve any text`);
      return;
    }
    ...
    try {
      const waitingNotice = new import_obsidian.Notice(
      ...
      const response = await this.openai.createChatCompletion({
        messages: [
          {
            role: "system",
            content: "You are an AnkiAssistant that will create flashcards to be used in the Anki App."
          },
          {
            role: "user",
          ...
          {
            role: "user",
            content: "Create flashcards from the following text:" + cleanedText
          }
        ],
      ...
      for (const flashcard of generatedFlashcards.trim().split("=======")) {
      ...
      new import_obsidian.Notice("Flashcards generated and added to Anki!");
    } catch (error) {
      console.error(error);
      new import_obsidian.Notice("An error occurred while generating flashcards");
    }
  }
  async getDeckId(name) {
    try {
  ...
  async createDeck(deckName) {
    try {
      ...
      console.error(error);
      new import_obsidian.Notice("An error occurred while creating the deck");
    }
  }
  async addCardToDeck(note, deckName) {
...
var FlashcardGeneratorSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("OpenAI API Key").setDesc("API key required for the OpenAI API").addText(
      (text) => text.setPlaceholder("Enter API key").setValue(this.plugin.settings.apiKey).onChange(async (value) => {
        this.plugin.settings.apiKey = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("Anki Deck Name").setDesc("Name of the Deck in Anki where flashcards will be added").addText(
      (text) => text.setPlaceholder("Enter Deck Name").setValue(this.plugin.settings.deckName).onChange(async (value) => {
        this.plugin.settings.deckName = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("OpenAI GPT Model Name").setDesc("Name of the OpenAI GPT model to use").addDropdown(
      (dropdown) => dropdown.addOption("gpt-4", "gpt-4").addOption("gpt-3.5-turbo", "gpt-3.5-turbo").addOption("text-davinci-003", "text-davinci-003").setValue(this.plugin.settings.modelName).onChange(async (value) => {
        this.plugin.settings.modelName = value;
        await this.plugin.saveSettings();
      })
    );
    new import_obsidian.Setting(containerEl).setName("OpenAI GPT Model Prompt").setDesc("User Prompt to use for the OpenAI GPT model").addText(
